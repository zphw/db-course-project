...
router.get('/spending/:start?/:end?', authCustomer, async (req, res) => {
    ...
    try {
        let totalStart, intervalStart;
        if (req.params.start) {
            totalStart = intervalStart = new Date(req.params.start).toISOString().split('T')[0];
        } else {
            totalStart = intervalStart = new Date();

            totalStart.setMonth(totalStart.getMonth() - 12);
            totalStart = totalStart.toISOString().split('T')[0];
            intervalStart.setMonth(intervalStart.getMonth() - 6);
            intervalStart = intervalStart.toISOString().split('T')[0];
        }

        let totalEnd, intervalEnd;
        if (req.params.end) {
            totalEnd = intervalEnd = new Date(req.params.end).toISOString().split('T')[0];
        } else {
            totalEnd = intervalEnd = new Date().toISOString().split('T')[0];
        }

        const data: {total: number, interval_spending: []} = {
            total: await spendingTotal(email, totalStart, totalEnd),
            interval_spending: await spendingInterval(email, intervalStart, intervalEnd),
        };
